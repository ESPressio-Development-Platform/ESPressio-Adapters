...
class AdapterHostRuntime final : public ESPressio::System::Execution::IExecutionProvider,
    ...
    std::array<Context,16> _contexts{};
...
public:
    ...
    public:
        ...
        Result Wait(std::uint32_t milliseconds=UINT32_MAX) noexcept override {
            ...
            if(milliseconds==UINT32_MAX)
                _ready.wait(lock,[&]{return _set;});
            else if(!_ready.wait_for(
                        lock,std::chrono::milliseconds(milliseconds),[&]{return _set;}))
    ...
    ~AdapterHostRuntime(){
        ...
        ESPressio::System::Execution::ResetProvider();
        ESPressio::System::Synchronization::ResetProvider();
    }
    void Pause(){
    ...
    Result Join(Handle h) override {
        if(h==_current||!h||h>_contexts.size())
            return Result::Failed(Status::InvalidArgument);
        auto& c=_contexts[h-1];
        assert(c.Used);
        c.Thread.join();
        c.Used=false;
        ++Joined;
    ...
    Result Destroy(Handle) override { std::abort(); }
    ...
    void SleepMilliseconds(std::uint32_t n) override {
        std::this_thread::sleep_for(std::chrono::milliseconds(n));
    }
    void Yield() override { std::this_thread::yield(); }
    bool SupportsProcessorAffinity() const noexcept override { return true; }
    std::unique_ptr<ESPressio::System::Synchronization::ISignal> CreateBinarySignal(
        bool set=false) override {
        if(FailSignal) return {};
        ++Signals;
        return std::make_unique<Signal>(*this,set);
    }
};

inline void WaitUntil(const std::atomic<unsigned>& value,unsigned expected){
    const auto limit=std::chrono::steady_clock::now()+std::chrono::seconds(3);
